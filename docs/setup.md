...
```bash
# marketplace を追加（GitHub owner/repo 形式。privateでもgit認証があれば可）
...
```
> private repo の場合：手動install/更新は既存のgit認証（`gh auth login` / SSH）で動く。バックグラウンド自動更新を効かせるなら `export GITHUB_TOKEN=...` を設定。
