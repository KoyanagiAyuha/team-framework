...
| 複数ファイル・ロジック変更 | Criticチームメイトにレビュー依頼／後半パイプラインの検証ゲート |
| アーキテクチャ変更 | Criticレビュー＋エンジニア承認 |

> Criticは**実コードを Read し、可能なら実行して裏取り**する。自己申告（要約）だけで合格させない。設計から作り直すべきなら `needsRedesign` を立て、Orchestratorが前半に戻して再計画する。
