...
>
> ゲートの中身（`worker-critic.mjs`）は3段: **① Worker が実装 → ② 収集役（sonnet・判断ゼロ）がテスト/tsc を実行し exit code＋失敗抜粋に圧縮 → ③ Critic（fable）がコードを Read し、②の証拠を使って合否判定**。テストの再実行はしない（②の証拠を使う）。粒度の背圧は2段——目安（約400行）超は**助言のみで差し戻さない**（サイズを理由に落とさない）／**安全弁（約1800行＝1パスで読み切れない容量）超のときだけ** Critic を呼ぶ前に `needsRedesign` で差し戻す。分割は行数合わせでなく自然な継ぎ目で。Criticは自己申告（要約）だけで合格させない。設計から作り直すべきなら `needsRedesign` を立て、Orchestratorが前半に戻して再計画する。
